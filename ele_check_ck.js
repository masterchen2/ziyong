...
async function _0x179175(data, context, options) {
    let response = await runOne(context, options);
    ...
    if (response && response.data) {
        ...
        if (responseData.code === 3000) {
            ...
            if (userEnvironment) {
                if (userEnvironment.id) {
                    await updateEnv11(updatedEnvironment, userEnvironment.id, userEnvironment.remarks, "elmqqck");
                } else {
                    await updateEnv(updatedEnvironment, userEnvironment._id, userEnvironment.remarks, "elmqqck");
                }
            }
